...
> calls the configured LLM. The default local Ollama provider has no hosted token
> charge; optional remote providers may charge for requests.
...
| `rephrase_chunks` fails | Ollama URL or model mismatch | Keep `OLLAMA_API_BASE=http://ollama:11434/v1`, set `OLLAMA_DEFAULT_MODEL` in `.env`, and run `docker compose exec ollama ollama list` |
