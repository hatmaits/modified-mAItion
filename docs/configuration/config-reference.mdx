...
- `model_config`: chat/rephrase model ID (`qwen3:4b` in the example environment)

For Ollama, the backend reads `OLLAMA_API_BASE=http://ollama:11434/v1` and
`OLLAMA_API_KEY=ollama` from `.env.rag`. The key is a compatibility placeholder.
The `/v1` path is required by the backend's OpenAI-compatible client.
