...
- `OLLAMA_BASE_URL`: native Ollama root URL (default: `http://ollama:11434`)
- `OLLAMA_DEFAULT_MODEL`: model pulled at startup and selected in OpenWebUI (default: `qwen3:4b`)
- `OLLAMA_CONTEXT_LENGTH`: context window configured on the Compose-managed Ollama service (default: `8192`)

Do not append `/v1` to `OLLAMA_BASE_URL`. OpenWebUI uses Ollama's native API.
The RAG backend has a separate `/v1` URL in `.env.rag`.
